...
f3 = OptimizationFunction((pol, params) -> optCrit(pol[1], pol[2], 0.0, pol[3], init; params = params, T = T, burn = burn, draw = draw, unconstr = true))
prob3 = Optimization.OptimizationProblem(f3, x0, params, lb = [minimum(grid[:Z]), 0, 0.0], ub = [maximum(grid[:Z]), 5.0, 1.0])
...
f4 = OptimizationFunction((pol, params) -> optCrit(pol[1], pol[2], pol[3], pol[4], init; params = params, T = T, burn = burn, draw = draw, unconstr = true))
prob4 = Optimization.OptimizationProblem(f4, x0, params, lb = [minimum(grid[:Z]), 0.0, 0.0, 0.0], ub = [maximum(grid[:Z]), 5.0,1.0, 1.0])
...
function callback4(state, loss_val)
    ...
    push!(ui_guesses4, state[3])
    push!(tax_guesses4, state[4])
    push!(lossvals4, loss_val)
    return false
end

sol4 = solve(prob4, NLopt.G_MLSL_LDS(); local_method = NLopt.LN_SBPLX(), callback = callback4, maxeval = 3000, local_maxiters = 200)
